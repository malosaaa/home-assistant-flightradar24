...
class FlightProcessor:
    ...
    def remove_track(self, number: str) -> dict | None:
        ...
        for flight_id, flight in self._tracked.items():
            if (
                number == flight.get("aircraft_registration")
                or number == flight.get("flight_number")
                or number == flight.get("callsign")
            ):
        ...
        if reg_numbers:
            ...
            for obj in flights:
                self._update_flights_data(
                    obj, current, self._tracked, FlightType.TRACKED
                )
            ...
            for fid, new_data in current.items():
                if new_data.get("tracked_type") == "schedule":
                    ...
                    for old_data in self._tracked.values():
                        old_number = old_data.get("flight_number") or old_data.get(
                            "callsign"
                        )
                        if (
                            old_number == number
                            and old_data.get("tracked_type") == "live"
                        ):
                            keys_to_remove.append(fid)
                            # Fire an event to Home Assistant for automations!
                            self._event_manager.add_events(
                                EVENT_TRACKED_ARRIVED_GATE, [old_data]
                            )
                            break
    ...
    def _update_flights_data(
        ...
        last_position = (
            previous_flight.get("on_ground") if previous_flight is not None else None
        )
        previous_closest_distance = (
            previous_flight.get("closest_distance")
            if previous_flight is not None
            else None
        )
        if (
            tracked is not None
            and obj.id in tracked
            and self._is_valid(tracked[obj.id])
        ...
        if flight is not None:
            ...
            flight["closest_distance"] = min(
                new_distance,
                previous_closest_distance
                if previous_closest_distance is not None
                else new_distance,
            )
    ...
    def _takeoff_and_landing(
        ...
        if (
            ...
            or position is None
            or last_position == position
        ):
            return
        if position == 0:
            self._event_manager.add_events(
                EVENT_AREA_TOOK_OFF
                if FlightType.IN_AREA == sensor_type
                else EVENT_TRACKED_TOOK_OFF,
                [flight],
            )
        else:
            self._event_manager.add_events(
    ...
    def _get_flight_data(self, flight: dict) -> dict[str, Any] | None:
        ...
        return {
            ...
            "aircraft_photo_small": get_value(
                flight, ["aircraft", "images", "thumbnails", 0, "src"]
            ),
            "aircraft_photo_medium": get_value(
                flight, ["aircraft", "images", "medium", 0, "src"]
            ),
            "aircraft_photo_large": get_value(
            ...
            "airport_origin_code_iata": get_value(
                flight, ["airport", "origin", "code", "iata"]
            ),
            "airport_origin_code_icao": get_value(
                flight, ["airport", "origin", "code", "icao"]
            ),
            "airport_origin_country_name": get_value(
                flight, ["airport", "origin", "position", "country", "name"]
            ),
            "airport_origin_country_code": _get_country_code(
                get_value(flight, ["airport", "origin", "position", "country", "code"])
            ),
            "airport_origin_city": get_value(
                flight, ["airport", "origin", "position", "region", "city"]
            ),
            "airport_origin_timezone_offset": get_value(
                flight, ["airport", "origin", "timezone", "offset"]
            ),
            "airport_origin_timezone_abbr": get_value(
                flight, ["airport", "origin", "timezone", "abbr"]
            ),
            "airport_origin_terminal": get_value(
                flight, ["airport", "origin", "info", "terminal"]
            ),
            "airport_origin_latitude": get_value(
                flight, ["airport", "origin", "position", "latitude"]
            ),
            "airport_origin_longitude": get_value(
                flight, ["airport", "origin", "position", "longitude"]
            ),
            "airport_destination_name": get_value(
                flight, ["airport", "destination", "name"]
            ),
            "airport_destination_code_iata": get_value(
                flight, ["airport", "destination", "code", "iata"]
            ),
            "airport_destination_code_icao": get_value(
                flight, ["airport", "destination", "code", "icao"]
            ),
            "airport_destination_country_name": get_value(
            ...
            "airport_destination_city": get_value(
                flight, ["airport", "destination", "position", "region", "city"]
            ),
            "airport_destination_timezone_offset": get_value(
                flight, ["airport", "destination", "timezone", "offset"]
            ),
            "airport_destination_timezone_abbr": get_value(
                flight, ["airport", "destination", "timezone", "abbr"]
            ),
            "airport_destination_terminal": get_value(
                flight, ["airport", "destination", "info", "terminal"]
            ),
            "airport_destination_latitude": get_value(
                flight, ["airport", "destination", "position", "latitude"]
            ),
            "airport_destination_longitude": get_value(
